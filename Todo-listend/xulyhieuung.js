...
//1. hiệu ứng phần input task(trỏ vào task Name)
...
//3. hiệu ứng đường line di chuyển phần menu
    tabName.forEach(item => {
        ...
        item.addEventListener('click', function(){
            ...
            document.querySelectorAll('.tabContainer ul').forEach(ul => {
                if(ul.dataset.pane === item.id){
                    document.querySelector('.tabContainer ul.active')?.classList.remove('active')
